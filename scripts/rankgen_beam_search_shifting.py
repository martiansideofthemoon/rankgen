...
def token_beam_search(contexts, scorer, beam_size=3, temperature=1.0, top_p=0.9, num_tokens=5, num_samples=10, max_length=115):
    ...
    for ctx in contexts:
        ...
        num_ctx_tokens = ctx_tokens.size()[1]
        ...
        while True:
            ...
            for beam in beams:
                ...
                for tokens, text, eos in zip(curr_outs['sequences'][:, num_input_tokens:], curr_outs_text, is_eos):
                    # update all_outs
                    prefix_tokens = torch.squeeze(beam['prefix_tokens'])
                    prefix_tokens = prefix_tokens.to(device)
                    new_prefix_tokens = torch.unsqueeze(torch.cat([prefix_tokens, tokens], dim=0)[-num_ctx_tokens:], dim=0)
                    all_outs.append({
